...
def regionDataScraping():
    ...
    df = pd.read_csv(json_url,index_col=0)
    df.reset_index(level=0, inplace=True)
    df = df.rename(columns={'Region / الجهة': 'Region', 'Total Cases / إجمالي الحالات': 'Confirmed', 'Active Cases / الحالات النشطة': 'Recovered','Total Deaths / إجمالي الوفيات': 'Deaths', 'Total Recovered / إجمالي المعافين': 'Active'})
